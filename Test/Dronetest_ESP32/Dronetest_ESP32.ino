...
void loop() {
    ...
    if (client) {
        ...
        if (request.indexOf("GET /off") != -1) {
            ...
            if (valueIndex != -1) {
                String throttleValue_string = request.substring(valueIndex + 6);
                throttleValue_string = throttleValue.substring(0, throttleValue.indexOf(' '));
                Serial.println("Throttle Value: " + throttleValue);
                throttleValue= throttleValue.toInt();
            }
