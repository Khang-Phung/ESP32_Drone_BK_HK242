...
void loop() {
  ...
  if (commandValue != 1) {
    ...
    /*CalculatePID3( Roll_set,  KalmanAngleRoll,  &pre_error_roll, &pre_error_roll2,  Kp_roll,  Ki_roll,  Kd_roll,  dt,  u_pre_roll);
    CalculatePID3( Pitch_set,  KalmanAnglePitch,  &pre_error_pitch, &pre_error_pitch2,  Kp_pitch,  Ki_pitch,  Kd_pitch,  dt,  u_pre_pitch);

    CalculatePID3( RateRoll_set,  Rate_Roll,  &pre_error_roll_gyro, &pre_error_roll_gyro2,  Kp_roll_gyro,  Ki_roll_gyro,  Kd_roll_gyro,  dt,  u_pre_roll_gyro);
    CalculatePID3( RatePitch_set,  Rate_Pitch,  &pre_error_pitch_gyro, &pre_error_pitch_gyro2,  Kp_pitch_gyro,  Ki_pitch_gyro,  Kd_pitch_gyro,  dt,  u_pre_pitch_gyro);
    */
    ...
    if (throttleValue < 1430){integral_roll = 0 ;integral_pitch = 0;}
    if (throttleValue < 1430){integral_roll_gyro = 0 ;integral_pitch_gyro = 0;}
    ...
    computeMotorInputs(throttleValue, output_roll_rate, output_pitch_rate, 0);  //
